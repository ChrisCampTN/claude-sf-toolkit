...
- GitHub is the sole DevOps/work-tracking backend (DevOps Center support removed in v2.0.0): GitHub Issues for tracking, GitHub Actions for CI/CD, PR-based promotion
- `config/sf-toolkit.json` → `devops.backend` is always `"github-actions"`
...
- After cross-cutting work-tracking changes, grep `DevOps Center` across `commands/` and `agents/` — zero hits expected (DC support removed in v2.0.0)
- If a change touches scripts, also run `node scripts/test-resolve-cache.js`

## Validation & Testing

### Automated (pre-commit)
- `node scripts/validate-plugin.js` — structural checks (JSON, versions, frontmatter, stale refs, hooks, backlog workflow file)
